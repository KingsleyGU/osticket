...
if(($subnav=$nav->getSubMenu()) && is_array($subnav)){
    ...
    foreach($subnav as $k=> $item) {
        ...
        echo sprintf('<li><a class="%s" href="%s" title="%s" id="%s" target="_blank" target="%s" download="%s">%s</a></li>',
                $class, $item['href'], $item['title'], $id,$item['target'],$item['download'], $item['desc'] );
