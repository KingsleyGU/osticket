...
class Ticket {
   ...
   function getCRMPhoneNumber() {
    ...
    function getAllPrintableAttachments()
    {
      ...
      foreach ($attachments as $key => $attachment) {
          ...
          elseif(substr($attachment['name'], 0, 7 ) == "image00")
                unset($attachments[$key]);
   ...
   function onOverdue($whine=true, $comments="") {
    ...
    function postReply($vars, &$errors, $alert=true, $claim=true) {
        ...
        if($vars['emailreply']==2 ||$vars['emailreply']==1)
        {
            ...
            {  
                ...
                if(isset($thisstaff)&&$thisstaff->getSignature())
                $responseBody = $responseBody.$response->ht['body'].$thisstaff->getSignature();
    ...
    function update($vars, &$errors) {
        ...
        $fields['note']     = array('type'=>'text',     'required'=>0, 'error'=>__('A reason for the update is required'));
